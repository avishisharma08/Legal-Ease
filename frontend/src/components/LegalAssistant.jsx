...
export default function LegalAssistant({ initialPrompt }) {
  ...
  const handleSendMessage = async (textToSend) => {
    ...
    try {
      // Build history for context (exclude the very first greeting)
      ...
      console.warn('Backend chat failed, falling back to local guidance:', err);
      // Helpful fallback in case backend is offline or rate-limited
      let responseText = "";
      ...
      if (lower.includes('indemnification') || lower.includes('indemnify')) {
        ...
        responseText = `### ⚖️ Non-Compete Enforceability\n\nNon-compete clauses restrict your ability to work for competitors or start a similar business within a specified geographic area and time frame.\n\n> 📌 **Recent Legal Trends:**\nCourts and regulators are increasingly cracking down on overly broad non-competes as unfair trade restraint.\n\n**What to Watch For:**\n- **Duration:** Anything over 1 year is generally considered restrictive for contractors.\n- **Scope:** Ensure the non-compete is limited strictly to direct competitors, not an entire industry.`;
      } else {
        responseText = `### ⚠️ Notice: Live AI Backend Unavailable\n\nCould not reach the live Gemini endpoint (\`${err.message}\`).\n\n**Suggestions:**\n1. Ensure the FastAPI backend is running (\`python -m uvicorn main:app --reload --port 8000\`).\n2. Verify that \`GEMINI_API_KEY\` is configured in \`backend/.env\`.\n3. In the meantime, try one of the prompt suggestion chips below!`;
      }
